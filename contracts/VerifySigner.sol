...
contract VerifySigner is Authorized {
   ...
   */
  function verify(uint256 _tokenID, bytes memory _signature)
    internal
    view
    returns (bool)
  {
   ...
   */
  function getAddressSigner(string memory _tokenID, bytes memory _signature)
    private
    view
    returns (address)
  {
  ...
  function createHash(string memory _tokenID) private view returns (bytes32) {
    return
      keccak256(abi.encodePacked(address(this), msg.sender, _tokenID, secret));
  }
